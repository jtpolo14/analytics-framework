...
$REGION = "us-central1"
$JOB_NAME = "transformer-task-cleaner"
$IMAGE = "us-central1-docker.pkg.dev/dulcet-iterator-344116/cloud-run-source-deploy/transformer-task-cleaner:latest"
$JOB_YAML = "job.yaml"
...
function replace_job {
    Write-Host "Replacing Cloud Run job..."
    gcloud run jobs replace $JOB_YAML --region $REGION --project $PROJECT_ID
}

function update_job {
    Write-Host "Updating Cloud Run job..."
