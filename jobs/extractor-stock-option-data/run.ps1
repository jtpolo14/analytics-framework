...
$REGION = "us-central1"
$JOB_NAME = "job-extractor-stock-option-data"
$IMAGE = "us-central1-docker.pkg.dev/dulcet-iterator-344116/cloud-run-source-deploy/job-extractor-stock-option-data:latest"
$JOB_YAML = "job.yaml"
...
function update_job {
    ...
    gcloud run jobs update $JOB_NAME --region=$REGION --project=$PROJECT_ID --add-cloudsql-instances=$CLOUD_SQL_INSTANCE --max-retries=0 --set-secrets="DB_PASSWORD=db-password:latest,FMP_API_KEY=fmp-api-key:latest"
}
